...
from bot.job.api.serializer import JobSerializer
from bot.job.models import Job
from bot.utils.constants import AUTH_URL, POST_TWEET_URL, TOKEN_URL, TWEET
from bot.utils.helper import generate_code_challenge, generate_code_verifier, make_token
...
class JobViewset(viewsets.GenericViewSet):
    ...
    def __tweet(self, token):
        ...
        payload = TWEET.format(
            job.position,
            job.company,
            job.location,
            job.date.strftime("%d-%m-%Y") if job.date else "NA",
            job.salary if job.salary else "NA",
            job.url,
        )
